...
每个快照包含约70个字段：
- 流动性指标（净流动性、RRP、TGA等）
- 货币环境（DXY、USDJPY、实际利率等）
- 经济周期判断
- 13个资产的RS Z-Score和RS动量
- 4个子评分和综合评分
- 领先指标信号
- 当日SPY/QQQ/IWM收盘价
- 后续1日/5日/20日收益（自动更新）
...
1. 每日查看仪表盘后，点击侧边栏的"💾 保存今日快照"
2. 系统会自动检测是否已保存，避免重复
3. 定期点击"📈 更新历史收益"来计算历史快照的后续表现

### 数据存储

- 数据保存在 `data/daily_snapshots.csv`
- 建议将此文件提交到GitHub仓库以持久化
- 每日快照约1KB，1年约250KB
